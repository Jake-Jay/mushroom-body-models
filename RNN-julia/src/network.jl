...
#SBATCH -n 1
#SBATCH -a 0-11
#SBATCH -o network-%j.out

#use the passive data from data_1
...
grid = [
    "lr" => [1f-5, 2f-5, 5f-5, 1f-4, 2f-4, 5f-4, 1f-3, 2f-3, 5f-3, 1f-2, 2f-2, 5f-2],
]
args = myparam(gridparams(grid))
@unpack lr = args
...
Flux.@epochs 2000 Flux.train!(ps, data, opt, Tracker.gradient; cb, logger, verbose = true) do x, y, weight
...
Base.Filesystem.mkpath("result_grid_search")
cd("result_grid_search")

file = matopen(string("model_", data_index, "_", args["lr"], ".mat"), "w")
...
file = matopen(string("pred_y_", data_index, "_", args["lr"], ".mat"), "w")
